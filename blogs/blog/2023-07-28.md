---
title: 开源LLM模型及社区项目调研及试验性部署
date: 2023-07-28 20:50:46
author: danc yuany3721
tags: 
    - LLM
categories: 
    - blog
...
初步选取符合要求的项目有两个：[langchain-ChatGLM：基于本地知识库的 ChatGLM 等大语言模型应用实现](https://github.com/chatchat-space/langchain-ChatGLM)、[闻达：一个大规模语言模型调用平台](https://github.com/wenda-LLM/wenda)。

尽管闻达提供了更灵活强大的API接口，提供了auto脚本的插件扩展方式，但它当前的用户WebUI只提供了类似new bing的功能，不提供基础的LLM对话功能，加之进行首次试验性部署，本次选择langchain-ChatGLM进行部署，基础模型选择[`ChatGLM2-6B`](https://github.com/THUDM/ChatGLM2-6B)，知识库向量化模型选择[`m3e-base`](https://huggingface.co/moka-ai/m3e-base)。
...
# 创建虚拟环境
python3 -m venv langchain-ChatGLM
# 激活虚拟环境
source langchain-ChatGLM/bin/activate
# 安装依赖
...
```bash
sudo mkdir /data/knowledge_base
sudo chmod  777 -R /data/knowledge_base
```
```python
# 本地embedding模型存储位置
embedding_model_dict = {
    ... ...,
    "m3e-base": "/data/model/m3e-base",
}
# 模型配置
llm_model_dict = {
    ... ...,
    "chatglm-6b": {
        ... ...,
        "pretrained_model_name": "/data/model/chatglm-6b",
        ... ...,
        ... ...
    },
    "chatglm-fitness-RLHF": {
        ... ...,
        "pretrained_model_name": "/data/model/chatglm-fitness-RLHF",
        ... ...,
    },
    "chatglm2-6b": {
        ... ...,
        "pretrained_model_name": "/data/model/chatglm2-6b",
        ... ...,
    },
}
# LLM 名称
...
修改`api.py`的默认端口号
```python
    parser.add_argument("--port", type=int, default=50079)
```
...
修改`vite.config.ts`端口号

```bash
# 打开防火墙
sudo ufw allow 50079:50080/tcp
sudo ufw reload
# 运行webui
npm run dev
